...
##
#Hey listen! This is bare minimum. Only does what the spec-sheet asks, nothing more. Wanted to implement better safeties.
#Actually, it's kind of a crufty mess. I don't like it. I wish I had more time to clean it up and work on it.
#Will probably rewrite as version 2.0 at some point.
#Spent... longer than I want to admit on this. 
#3 hrs Saturday "I'll totally get this done." Nope.
#3 hrs Sunday "I'm almost done, it'll be great." Nope.
#Monday night, start working on it, thinking I'll get it done in time for the deadline.
#Cue 8-ish hours straight trying to get the pissing thing to work with all the safeties I had written in.
#Cue 4 more hours scrapping those safeties and just straight debugging.
#OMGWTFBBQ
#So here it is.
##
...
IDENT_NUM_MAX => 5000,
DEPARTMENT_MAX => 10,
YEARS_MIN => 1, #We're counting partial years as whole years, for the sake of simplicity.
YEARS_MAX => 40,
};
...
sub main {
	...
	#sleep 1; #Argh. Ran into the same sleep buffer issue I had last semester.
	#Sleep prevents the immediately preceeding print statement from issuing
	#until after the sleep has completed. No idea why. Uncomment if you dare.
}
...
sub setContinueInt {
	...
	do {
		...
		if (!($cleanInput eq INT_FAILURE) && $cleanInput == $dirtyInput) {
			debugPrint("cleanInput",$cleanInput);
...
#	my $recordConditionFlag;
	do {
		...
		if (!($cleanInput eq INT_FAILURE) && $cleanInput == $dirtyInput) {
			...
			print "Input rejected. Either you entered a non-number,";
			print "or the number was out of range. Range is 0-" . IDENT_NUM_MAX;
			setContinueInt();
		}
...
sub getEmployeeDept {
	...
	do {
		...
		if (!($cleanInput eq INT_FAILURE) && $cleanInput == $dirtyInput) {
			...
			print "Input rejected. Either you entered a non-number,";
			print "or the number was out of range. Range is 0-" . DEPARTMENT_MAX;
			setContinueInt();
		}
...
sub getEmployeeYears {
	...
	do {
		...
		chomp ($dirtyInput = <STDIN>); 
		debugPrint("dirtyIn",$dirtyInput);
		$cleanInput = sanitizeNum(YEARS_MIN,YEARS_MAX,$dirtyInput);
		debugPrint("cleanIn",$cleanInput);
		...
		if (!($cleanInput eq INT_FAILURE) && $cleanInput == $dirtyInput) {
			...
			print "Input rejected. Either you entered a non-number,";
			print "or the number was out of range. Range is " . YEARS_MIN . "-" . YEARS_MAX;
			setContinueInt();
		}
	} while ($continueInt == TRUE);
...
sub fetchDeptTotalYears {
	...
	foreach my $employee (@employeesRecord) {
		...
		@$employee[IDX_EMPLOYEE_DEPT] == $currentRecord[IDX_EMPLOYEE_DEPT]) {
